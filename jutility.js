...
function replace_side_panel_platform_omni() {
    ...
    Object.keys(array_dict).forEach(function(key_name) {
        ...
        array_subli = _.sortBy(array_subli, function(D) {
            return parseFloat(D['rank']) || 0
        })
        array_subli.reverse()
        L.push({
            parent_name: key_name,
            links: _.map(array_subli, function(D) {
                return {
                    attributes: {
                        href: D['url'],
                        text: D['name']
...
function add_floating_chat_box_base(chat_id, message_content_id, message_box_id, favicon, small_chat_date, chat_title, small_chat_box_style, small_chat_style) {
    ...
    chat_icon = $("<div>", {
        "id": chat_id,
        class: 'small-chat',
        "style": small_chat_style
    }).append(badge_counter).append(badge_icon)
    ...
    heading = parent_heading.append($("<small>", {
        "class": "chat-date pull-right"
    }).text(small_chat_date)).append($("<span>", {}).text(chat_title))
    message_content = $("<div>", {
        "class": "content message_content",
    ...
    form_chat = $("<div>", {
        ...
        "type": "text",
        "class": "form-control message_box",
        "id": message_box_id
    })).append($("<span>", {
        "class": "input-group-btn"
    }).append($("<button>", {
        "class": "btn btn-primary message_send",
        "type": "button"
    }).text("Send"))))
    chat_session = $("<div>", {
        "class": "small-chat-box fadeInRight animated",
        "style": small_chat_box_style
    }).append(heading).append(message_content).append(form_chat)
...
function list_progress_bar_list_element_thick(title_text, id, percentage, parent_identifier, color, metric_text) {
    ...
    var metric_text_div = $("<small>", {
        'class': 'pull-right percentage_text'
    }).text(metric_text)



    var progress_bar_parent_div = $("<div>", {
...
function metric_header_create_label(title_text, sub_title, metric_text, sub_metric_text, id, label_color, params) {
    ...
    var outer_div_one = $("<div>", {
        "class": "col-md-2 " + title_text,
        "id": id
    });
    var outer_div_two = $("<div>", {
        "class": "ibox float-e-margins"
    });
    var inner_div_one = $("<div>", {
        "class": "ibox-title"
    });
    var elem_one = $("<span>", {
    ...
    var inner_div_two = $("<div>", {
        "class": "ibox-content"
    });
    var elem_three = $("<h1>", {
        "class": "no-margins metric_text kpi"
    }).text(metric_text)
    //var elem_four = $("<div>", {"class": "stat-percent font-bold text-success sub_metric_text"}).text(sub_metric_text)
    var elem_four = $("<span>", {
        "class": "label pull-right sub_metric_text " + label_color
    }).text(sub_metric_text)
...
function metric_header_create(title_text, sub_title, metric_text, sub_metric_text, id) {
    ...
    var elem_three = $("<h1>", {
        "class": "no-margins metric_text"
    }).text(metric_text)
    var elem_four = $("<div>", {
        "class": "stat-percent font-bold text-success sub_metric_text"
    }).text(sub_metric_text)
    var elem_five = $("<small>", {
        "class": "sub_title"
    }).text(sub_title)
...
function datatables_data_get_from_api(table_id) {
    ...
    return $(table_id).DataTable().api().rows({
        page: "current"
    }).data();
}
...
function callback_function_sum_datatables(variable_name) {
    var api = this.api();
    variable_name = api.column(variable_name + ":name", {
        page: 'current'
    }).data().sum()
    return api
}
...
function guesty_reservation_data_pull() {
    ...
    results = $.ajax({
        ...
        async: false,
        data: {
            'viewId': '5616071779035e0e0096290c'
        },
        headers: {
...
function guesty_listing_data_pull() {
    response_result = $.ajax({
        type: "GET",
        url: 'https://superhostuser.herokuapp.com/api/v2/listings',
        dataType: 'json',
        async: false,
        headers: {
            "Authorization": "Basic " + btoa("57b6349a1f211d3c4b2b4c886c5632c7:1b8e3bd1f42ce6b054868cd47dc0412f")
        },
        data: {
            limit: 50
        }
...
function gspread_table_tasks_generate(gspread_array_data, completed_tasks, current_tasks) {
    ...
    gspread_array_data.forEach(function(gspread_dict) {
        filtered_completed_tasks = completed_tasks.filter(function(complete_dict) {
            return complete_dict['content'].toLowerCase().indexOf(gspread_dict['Task'].toLowerCase()) != -1
        })
        filtered_completed_tasks_today = filtered_completed_tasks.filter(function(complete_dict) {
            return date_range_filter(complete_dict.completed_date, 'YY-MM-DD')
        })
        ...
        if (filtered_completed_tasks.length > 0) {
            last_completed = _.max(filtered_completed_tasks, function(complete_dict) {
                return moment(complete_dict.completed_date).valueOf();
            }).completed_date
        ...
        if (gspread_dict['status'] != 'Green') {
            ...
            filtered_current_tasks = current_tasks.filter(function(current_dict) {
                return current_dict['content'].toLowerCase().indexOf(gspread_dict['Task'].toLowerCase()) != -1
            })
            is_assigned = filtered_current_tasks.length > 0
            if (is_assigned) {
                filtered_current_tasks_with_due_date = filtered_current_tasks.filter(function(D) {
...
function todoist_completed_tasks_all_custom(todoist_api_token, since) {
    ...
    l.forEach(function(item) {
        tasks_array_customize_item(item)
    })
    return l
}
...
function todoist_tasks_pull_custom() {
    ...
    current_tasks.forEach(function(D) {
        D['task_date'] = D['due_date_utc']
    })
    completed_tasks.forEach(function(D) {
        D['task_type'] = 'completed'
    })
    completed_tasks.forEach(function(D) {
        D['task_date'] = D['completed_date']
    })



    current_completed_tasks = completed_tasks.concat(current_tasks) //combine both arrays together into one array
    current_completed_tasks.forEach(function(item) {
        tasks_array_customize_item(item)
    })
    current_completed_tasks.forEach(function(item) {
        labels_add_from_labels_dictionary(item, labels_dictionary)
    })
    current_completed_tasks.forEach(function(item) {
        project_name_append(item, projects_dictionary)
    })
    current_completed_tasks.forEach(function(item) {
        item['task_date_range'] = date_within_range_string_create(item['task_date'])
    })
...
function gspread_array_project_id_append(gspread_array_data, projects_dictionary) {
    ...
    projects_dictionary.forEach(function(D) {
        project_dictionary_reference[D['name']] = D['id']
    })
...
function todoist_tasks_pull_custom_gspread() {
    ...
    current_tasks.forEach(function(D) {
        D['age'] = age_calculate_from_todoist_task(D)
    })
    ...
    completed_tasks.forEach(function(D) {
        completed_task_start_time_end_time(D)
    })
    ...
    todoist_current = current_completed_tasks.filter(function(D) {
        return D['task_type'] == 'current'
    })
    todoist_completed = current_completed_tasks.filter(function(D) {
        return D['task_type'] == 'completed'
    })


    array_check_keys(current_completed_tasks, ['due_date_utc', 'priority', 'date_added', 'completed_date', 'age', 'task_date'])
    return {
        todoist_current: todoist_current,
        todoist_completed: todoist_completed,
        todoist: current_completed_tasks,
        gspread: gspread_array,
        projects: projects_dictionary,
...
function datatables_column_add_formatting_from_type(new_dictionary) {
    ...
    if (new_dictionary.format == 'hyperlink') {
        new_dictionary.render = function(data, type, row, meta) {
            //title = row.name||row.title
...
function food_table() {
    function callback_function(data) {
        data.forEach(function(D) {
            D['calories'] = D['file_name'].split(' ')[0]
        })
...
function stock_table() {
    ...
    function update_stock_data() {
        ...
        stock_symbols = _.map(table_data, function(D) {
    ...
    function stocks_table(user) {
        return datatables_firebase({
            ...
            columns: [{
                    ...
                    data: 'total_share_return',
                    title: 'total_share_return',
                    render: function(data, type, row, meta) {
                        r = parseFloat(row.latestPrice) - parseFloat(row.average_cost)
...
function current_tasks_call_back(callback_array) {
    ...
    task_dates = Object.keys(_.groupBy(callback_array, function(D) {
        return moment(D['task_date']).format("MM/DD/YY")
    })).length
...
function measure_progress_bars(callback_array, progress_table) {
    $("td.progress_metric_measure").each(function(e, i) {
        ...
        table_data.forEach(function(D, row_number) {
            D['row_number'] = row_number
        })
...
function todoist_table_create_complete(array, table_id, metric_headers_update_list) {
    ...
    function run_refresh() {
        measure_progress_bars(Object.values(table.rows({
            page: "current"
        }).data()), progress_table)
